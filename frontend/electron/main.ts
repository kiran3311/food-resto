...
const getPrinterList = async (): Promise<Electron.PrinterInfo[]> => {
  if (!mainWindow) {
...
const printReceipt = async ({ html, printerName }: PrintReceiptPayload): Promise<void> => {
  const printers = await getPrinterList();
  const selectedPrinter =
    printerName?.trim() || printers.find((printer) => printer.isDefault)?.name || "";
...
app.whenReady().then(async () => {
  ...
  ipcMain.handle("printer:print-receipt", async (_event, payload: PrintReceiptPayload) => {
    try {
      ...
      const message =
        error instanceof Error ? error.message : "Silent printing failed.";
